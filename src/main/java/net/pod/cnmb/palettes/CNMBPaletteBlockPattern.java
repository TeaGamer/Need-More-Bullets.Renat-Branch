...
import static net.pod.cnmb.palettes.CNMBPaletteBlockPartial.ALL_PARTIALS;
import static net.pod.cnmb.palettes.CNMBPaletteBlockPartial.FOR_POLISHED;
import static net.pod.cnmb.palettes.CNMBPaletteBlockPattern.PatternNameType.*;

/**
 * Copy of PaletteBlockPattern from create
 * @see com.simibubi.create.content.decoration.palettes.PaletteBlockPattern
 */
public class CNMBPaletteBlockPattern {

    public static final CNMBPaletteBlockPattern
        ...
        LAYERED = create("layered", PREFIX).blockStateFactory(p -> p::cubeColumn)
                ...
                .connectedTextures(v -> new HorizontalCTBehaviour(ct(v, CNMBPaletteBlockPattern.CTs.LAYERED), ct(v, CNMBPaletteBlockPattern.CTs.CAP))),
    
        PILLAR = create("pillar", SUFFIX).blockStateFactory(p -> p::pillar)
                .block(ConnectedPillarBlock::new)
                .textures("pillar", "cap")
                .connectedTextures(v -> new RotatedPillarCTBehaviour(ct(v, CNMBPaletteBlockPattern.CTs.PILLAR), ct(v, CNMBPaletteBlockPattern.CTs.CAP)))
    ;

    public static final CNMBPaletteBlockPattern[] VANILLA_RANGE = { CUT, POLISHED, BRICKS, SMALL_BRICKS, LAYERED, PILLAR };

    public static final CNMBPaletteBlockPattern[] STANDARD_RANGE = { CUT, POLISHED, BRICKS, SMALL_BRICKS, LAYERED, PILLAR };

    static final String TEXTURE_LOCATION = "block/palettes/stone_types/%s/%s";

    private CNMBPaletteBlockPattern.PatternNameType nameType;
    ...
    private CNMBPaletteBlockPattern.IPatternBlockStateGenerator blockStateGenerator;
    ...
    private CNMBPaletteBlockPartial<? extends Block>[] partials;

    @OnlyIn(Dist.CLIENT)
    private RenderType renderType;

    private static CNMBPaletteBlockPattern create(String name, CNMBPaletteBlockPattern.PatternNameType nameType, CNMBPaletteBlockPartial<?>... partials) {
        CNMBPaletteBlockPattern pattern = new CNMBPaletteBlockPattern();
        pattern.id = name;
    ...
    public CNMBPaletteBlockPattern.IPatternBlockStateGenerator getBlockStateGenerator() {
    ...
    public CNMBPaletteBlockPartial<? extends Block>[] getPartials() {
        return partials;
    }
    ...
    private CNMBPaletteBlockPattern blockStateFactory(CNMBPaletteBlockPattern.IPatternBlockStateGenerator factory) {
        blockStateGenerator = factory;
        return this;
    }

    private CNMBPaletteBlockPattern textures(String... textures) {
        this.textures = textures;
        return this;
    }

    private CNMBPaletteBlockPattern block(NonNullFunction<BlockBehaviour.Properties, ? extends Block> blockFactory) {
        this.blockFactory = blockFactory;
        return this;
    }

    private CNMBPaletteBlockPattern connectedTextures(Function<String, ConnectedTextureBehaviour> factory) {
    ...
    public CNMBPaletteBlockPattern.IBlockStateProvider cubeAll(String variant) {
    ...
    protected static CTSpriteShiftEntry ct(String variant, CNMBPaletteBlockPattern.CTs texture) {
    ...
    static interface IPatternBlockStateGenerator
            extends Function<CNMBPaletteBlockPattern, Function<String, CNMBPaletteBlockPattern.IBlockStateProvider>> {
    }
